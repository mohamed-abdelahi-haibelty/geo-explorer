...
export function ArticleFilters({
  ...
  const authorOptions = [{ value: "all", label: "Tous les auteurs" }, ...authors.map((a) => ({ value: a.id, label: a.name }))];
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
      ...
      <Select items={authorOptions} value={authorId ?? "all"} onValueChange={(value) => pushParams({ author: value === "all" ? undefined : String(value) })}>
        <SelectTrigger aria-label="Filtrer par auteur" className="w-full sm:w-48">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
